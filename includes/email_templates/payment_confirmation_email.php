...
try {
    ...
    if ($payment['email_sent'] == 0) {
        try {
            ...
            // Replace placeholders in the email body, including header/footer specific ones
            $email_body = str_replace('{{email_title}}', 'Payment Confirmation - Mackenny Assessment', $email_body); // New
            $email_body = str_replace('{{header_text}}', 'Payment Confirmation - Mackenny Assessment', $email_body); // New

            $email_body = str_replace('{{first_name}}', htmlspecialchars($payment['first_name']), $email_body);
            $email_body = str_replace('{{last_name}}', htmlspecialchars($payment['last_name']), $email_body);
